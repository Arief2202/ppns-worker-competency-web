        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" style="z-index: 0">
            <div class="bg-dark2 dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('schedule.worker.create.post') }}">@csrf      
                      <input type="hidden" name="schedule_id" id="schedule_id" value="{{ old('schedule_id', $schedule_id) }}">                  
                      ...
                      <input type="hidden" name="worker_id" id="worker_id" value="{{ old('worker_id') }}">                  
                        ...
                        <div class="mb-5">
                          ...
                          <div class="input-group">
                            <input type="text" class="form-control @error('worker') is-invalid @enderror" aria-label="Text input with dropdown button" value="{{ old('worker') }}" id="worker_show" disabled>
                          ...
                          <input type="hidden" name="worker" id="worker" value="{{ old('worker') }}" class="@error('worker') is-invalid @enderror">
                          @error('worker')
                          <div class="invalid-feedback">
                            {{ $message }}
                          </div>
    ...
    <x-slot name="script">
        <script type="text/javascript">
            $(document).ready( function () {
                ...
                @if($schedule_id && old('competency') && old('competency_id'))
                  select("{{ $schedule_id }}", "{{ old('competency') }}", "{{ old('competency_id') }}");
                @endif
            } );
